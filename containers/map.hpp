	...
	class map
	{
		...
		public:
			explicit map(key_compare const& comp = key_compare(),
				...
				size_(0)
			{
				this->ghost_node->parent = this->ghost_node_;
				this->ghost_node->left = this->ghost_node_;
				this->ghost_node->right = this->ghost_node_;
				this->ghost_node_->is_red = false;
				this->nil_node_->is_red = false;
				this->nil_node->parent = NULL;
				this->nil_node->left = NULL;
				this->nil_node->right = NULL;
				return;
			}
			...
			void erase(iterator first, iterator last)
			{
				...
				while (first != last)
				{
					...
					std::cout << "to_del : " << to_delete->first << std::endl;
					first++;
		...
		private:
			...
			ft::pair<iterator, bool> insert_to_root(value_type const& val)
			{
				...
				this->ghost_node->right = this->root_node_;
